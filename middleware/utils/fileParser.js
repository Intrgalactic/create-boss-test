...
const parseFile = async (fileBuffer, fileType) => {
    ...
    switch (fileType) {
        ...
        case "application/msword": return await parseDoc(fileBuffer);
            break;
        case "application/rtf": return await parseRtf(fileBuffer);
            break;
...
const parseRtf = async (fileBuffer) => {
    try {
        const rtfStream = new Readable();
        rtfStream._read = () => { }; // Required to initialize the readable stream
        rtfStream.push(fileBuffer); // Push the file buffer to the stream
        rtfStream.push(null); // Signal the end of the stream

        const parser = new RtfParser();
        const plainText = await parser.parse(rtfStream);
        return plainText;
    } catch (err) {
        console.error('Error parsing RTF file buffer:', err);
        throw err;
    }
}
