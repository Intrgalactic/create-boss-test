...
export function FileInputContainer({ file, setFile, textInput }) {
    ...
    return (
        <div className='file-input-container' onClick={() => { document.querySelector("input[type=file]").click() }} ref={fileRef} >
            {textInput === "" ? <input type="file" onChange={(e) => { setFile(e.target.files[0]) }} accept="text/plain,application/rtf,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.oasis.opendocument.text" ref={fileInputRef} /> : <input type="file" disabled />}
        </div>
    )
}